...
第3步：候选位置评估
  ...
  对于陡峭梯度（TOP词明显）：
    ✓ 与TOP词相邻 (距离1步) → 置信度0.90+ 最优
      └─ 进一步优化：如果有多个与TOP词相邻的格子，优先选择同时靠近"第二高相关词"的位置
         例：TOP词离该格子50px，但第二高相关词离该格子100px vs 200px → 选100px的
         这样既靠近TOP词，又为第二高相关词预留空间
    ✓ 与TOP词距离2步但几何<150px → 置信度0.85 很好
    ✓ 与TOP词距离3步 → 置信度0.70 可考虑
    ✗ 靠近低相关词 → 置信度<0.50 避免

    关键优化：即使最终选择是与TOP词相邻，也要看"低相关词之间的相对高低"
    例：「秦陵」对 秦长城90% > 故宫75% > 埃及20% > 泰姬陵15%
    → 与秦长城相邻是必选（90%远高于其他）
    → 但如果有多个与秦长城都相邻的格子，选择同时靠近故宫(75%)而不是埃及(20%)的那个
    → 这反映了"即使不是TOP词，也要尊重相对高低关系"
...
示例1 - 陡峭梯度（TOP词明显，含低相关词对比）：
题目：新词「正定古城」，已有词：[「北京」→92%, 「天津」→88%, 「杭州」→25%, 「巴黎」→5%]
分析：
  - 相关度排序：北京92% > 天津88% >> 杭州25% > 巴黎5%
  - 梯度特征：陡峭，北京/天津领先，与南方/国外有明显分割
  - TOP词决策：必须与北京或天津相邻（92% vs 88%都很高）
  - 关键优化：如果有多个与北京相邻的格子，优先选择同时靠近天津(88%)而不是杭州(25%)的
    原因：虽然都是非TOP词，但天津(88%)的相关度明显高于杭州(25%)，应该优先保留天津的聚集空间
  - 候选位置：
    * 格子A与北京相邻，同时距天津150px → 置信度0.92 (最优)
    * 格子B与北京相邻，但距杭州80px(近于天津200px) → 置信度0.88 (不如A)
  - 输出：{"selectedCell":"1,2","reasoning":"TOP词北京92%，次选词天津88%相关度接近且都高，选与北京相邻同时靠近天津的位置","confidence":0.92}
